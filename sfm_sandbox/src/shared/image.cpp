...
bool Image::loadData() {
  if (isLoaded()) {
    ...
    try {
      data_ = std::make_shared<cv::Mat>(cv::imread(
          path_,
          cv::IMREAD_COLOR));  // TODO make an interface between opencv so it can be swapped out in the future
...
void Image::generateThumbnailAndSet_() {
  thumbnail_resolution_ =
      cv::Size(thumbnail_width_, thumbnail_width_ * resolution_.height / resolution_.width);
