...
class FeatureDetector {
 protected:
  virtual void detect(const std::shared_ptr<cv::Mat>& image,
                      std::vector<cv::KeyPoint>& keypoints) = 0;
  virtual void compute(const std::shared_ptr<cv::Mat>& image, std::vector<cv::KeyPoint>& keypoints,
                       cv::Mat& descriptors) = 0;
  virtual void detectAndCompute(const std::shared_ptr<cv::Mat>& image,
                                std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors) = 0;
  virtual ~FeatureDetector(){};

 public:
  void detect(std::vector<std::shared_ptr<Image>> images,
  ...
  void compute(std::vector<std::shared_ptr<Image>> images,
               std::vector<std::vector<cv::KeyPoint>>& keypoints,
  ...
  void detectAndCompute(std::vector<std::shared_ptr<Image>> images,
                        std::vector<std::vector<cv::KeyPoint>>& keypoints,
                        std::vector<cv::Mat>& descriptors) {
    CHECK_EQ(images.size(), keypoints.size());
    CHECK_EQ(images.size(), descriptors.size());
